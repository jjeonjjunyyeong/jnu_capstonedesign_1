...
def hotel(result):
    ...
    # 인원입력칸 클릭
    try:
        driver.find_element(By.CSS_SELECTOR, '#occupancy-box > div > div > div > div.SearchBoxTextDescription__child').click()
        time.sleep(2)
    
        for i in range(3):
            time.sleep(2)
            driver.find_element(By.CSS_SELECTOR, '#occupancy-selector > div > div > div:nth-child(2) > div.sc-bdfBwQ.sc-gsTCUz.jImVoD > div.sc-bdfBwQ.sc-gsTCUz.bAJgph').click()
